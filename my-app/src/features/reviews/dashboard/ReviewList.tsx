...
const ReviewList: React.FC<IProps> = ({
  ...
  const handleSubmit = () => {
    ...
    toast.current?.show({
      severity: 'success',
      summary: 'Sukses',
      detail: 'Review u shtua me sukses!',
      life: 3000
    });
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ alignItems: "flex-start", marginTop: "15px", minHeight: "5cm" }}
      >
        <div style={{ width: "35cm", padding: "1cm", paddingBottom: "2cm" }}>
          <h3 style={{ color: "#1c2c3c" }}>
            <b>Reviews</b>
          </h3>
            ...
            <Row style={{ marginTop: "30px" }}>
              <Col>
                <div className="input-box">
                  <input
                    ...
                    onChange={(e) =>
                      setReview({ ...review, komenti: e.target.value })
                    }
                  />
              ...
              <Col xs={1}>
                <div className="input-box">
                  <button
                    className="submitbtn"
                    id="sendReviewBtn" // <-- shtuar për Selenium
          ...
          {filteredReviews.slice(0, displayedReviews).map((r) => (
                  ...
                  {isAdmin ? (
                    ...
                    r.id === localStorage.getItem("userId") && (
                      <div className="review-right">
                       ...
                       <button
  id={`delReviewBtn-${r.reviewId}`}
  className="submitbtn delbtn"
  onClick={() => deleteReview(r.reviewId)}
>
  <b><i className="pi pi-trash" /></b>
</button>
                      </div>
                    )
                  )}
          ...
          {filteredReviews.length > 5 && (
            <div style={{ textAlign: "center", margin: "10px" }}>
              {displayedReviews < filteredReviews.length ? (
                <button className="btn-link" onClick={seeMoreReviews}>
                  Show More
                </button>
              ) : (
                <button className="btn-link" onClick={seeLessReviews}>
                  Show Less
                </button>
              )}
            </div>
